...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A prompts/ folder exists with one prompt file per main interaction: career-timeline interview, job-search interview, new-application kickoff, and past-resume ingestion
- [ ] #2 just recipes exist to run each prompt through Claude Code (e.g. just interview-career, just interview-search, just new-application <company>), following the model/mode override pattern used by the existing review recipe
...
- [ ] #4 GETTING_STARTED.md is updated to point to the new recipes at each step instead of only describing free-form prompting
